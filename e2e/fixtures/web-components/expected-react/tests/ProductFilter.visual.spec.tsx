...
const referenceHtml = readFileSync(resolve(currentDir, "./ProductFilter.reference.html"), "utf-8");
const viewports = [
  {
    "name": "default",
    "width": 1440,
	...
	test.describe(`${viewport.name ?? `${viewport.width}x${viewport.height}`}`, () => {
		...
		for (const state of states) {
			test(`Visual Regression / ${state.name}`, async ({ page, mount }, testInfo) => {
				const snapshotName = `ProductFilter-${viewport.name ?? `${viewport.width}x${viewport.height}`}-${state.name}.png`;
				...
				if (!existsSync(snapshotPath)) {
					...
					const locator = page.locator(".filter-section").first();
