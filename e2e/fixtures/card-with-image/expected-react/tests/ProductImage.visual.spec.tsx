	...
	test.describe(`${viewport.name ?? `${viewport.width}x${viewport.height}`}`, () => {
		...
		for (const state of states) {
			test(`Visual Regression / ${state.name}`, async ({ page, mount }, testInfo) => {
				const snapshotName = `ProductImage-${viewport.name ?? `${viewport.width}x${viewport.height}`}-${state.name}.png`;
				...
				if (!existsSync(snapshotPath)) {
					...
					const locator = page.locator("img").first();
				...
				const component = await mount(<ProductImage src="/assets/shoe.png" alt="Trail shoe" />);
