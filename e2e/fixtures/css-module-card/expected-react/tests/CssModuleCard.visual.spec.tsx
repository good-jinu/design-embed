...
import { CssModuleCard } from "../CssModuleCard.view";

const currentDir = dirname(fileURLToPath(import.meta.url));
const referenceHtml = readFileSync(resolve(currentDir, "./CssModuleCard.reference.html"), "utf-8");
const viewports = [
  {
    "name": "default",
    "width": 1440,
...
for (const viewport of viewports) {
	for (const state of states) {
		...
		test("CssModuleCard matches source at " + viewportName + " / " + state.name, async ({ mount, page }) => {
			...
			const component = await mount(<CssModuleCard />);
			await applyState(page, state);
