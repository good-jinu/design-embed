...
const referenceHtml = readFileSync(resolve(currentDir, "./TailwindCard.reference.html"), "utf-8");
const viewports = [
  {
    "name": "default",
    "width": 1440,
	...
	test.describe(`${viewport.name ?? `${viewport.width}x${viewport.height}`}`, () => {
		...
		for (const state of states) {
			test(`Visual Regression / ${state.name}`, async ({ page, mount }, testInfo) => {
				const snapshotName = `TailwindCard-${viewport.name ?? `${viewport.width}x${viewport.height}`}-${state.name}.png`;
				...
				const component = await mount(<TailwindCard />);
