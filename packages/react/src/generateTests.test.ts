...
import { reactTestGenerator } from "./index.ts";
...
describe("React generateTests — snapshotPath integration", () => {
	test("generated spec calls toHaveScreenshot with baseline name when snapshotPath is set", () => {
		...
		const spec = result.files.find((f) => f.path.endsWith(".visual.spec.tsx"));
		assert.ok(spec, "expected a .visual.spec.tsx file");
		assert.ok(
